...
def glow_circle(dst, pos, radius, color, strength=1):
    ...
    for i in range(5, 0, -1):
        pygame.draw.circle(
            surf,
            (*color, int(8 * strength * (6 - i))),
            c,
            int(radius * (0.5 + i * 0.42)),
        )
    ...
    def draw(self, dst, offset):
        ...
        if self.kind == "spark":
            pygame.draw.line(
                ...
                (*color, int(230 * t)),
                p,
                p - safe_normal(self.vel) * self.size * 4,
                max(1, int(self.size * t)),
            )
        elif self.kind == "smoke":
            pygame.draw.circle(
                dst, (*color, int(75 * t)), p, int(self.size * (1.7 - t))
            )
        else:
            glow_circle(dst, p, self.size * t, color, t)
    ...
    def __init__(self, muted=False):
        ...
        try:
            ...
            for key, name in names.items():
                ...
                sound.set_volume(
                    0.32 if key in ("slash", "liquid1", "liquid2") else 0.42
                )
...
class Battle:
    ...
    def impact(self, pos, power=1, color=RED):
        ...
        self.hit_stop = max(self.hit_stop, 0.012 + power * 0.012)
    ...
    def add_pool(self):
        ...
        for i in range(count):
            ...
            points.append(
                Vec2(math.cos(angle), math.sin(angle)) * random.uniform(29, 52)
            )
        self.pools.append(
            Pool(Vec2(self.dummy.pos), points, random.random() * math.tau)
        )
        self.dark.stacks += 1
        self.sound.play(random.choice(("liquid1", "liquid2")))
        self.text(
            f"VIRA STACK  {self.dark.stacks}/10",
            self.dummy.pos + Vec2(0, -88),
            RED,
            True,
        )
    ...
    def start_portal(self, normal):
        d = self.dark
        d.mode, d.mode_timer, d.hidden = "portal", 0.45, False
        d.portal_timer = 9
        entry = Vec2(d.pos)
        self.portals.append(Portal(entry, normal, 0.8, 0.8))
        if abs(normal.x):
            exit_pos, exit_normal = (
                Vec2(ARENA.right if normal.x < 0 else ARENA.left, entry.y),
                normal,
            )
        else:
            exit_pos, exit_normal = (
                Vec2(entry.x, ARENA.bottom if normal.y < 0 else ARENA.top),
                normal,
            )
    ...
    def emerge(self):
        ...
        self.portals.append(
            Portal(Vec2(self.dash_target), Vec2(self.lock_offset), 0.9, 0.9, True)
        )
    ...
    def stab(self):
        ...
        locked_pos = Vec2(
            clamp(
                self.dummy.pos.x,
                ARENA.left + self.dummy.radius,
                ARENA.right - self.dummy.radius,
            ),
            clamp(
                self.dummy.pos.y,
                ARENA.top + self.dummy.radius,
                ARENA.bottom - self.dummy.radius,
            ),
        )
    ...
    def cast_fire(self):
        ...
        base = math.degrees(
            math.atan2(self.dummy.pos.y - d.pos.y, self.dummy.pos.x - d.pos.x)
        )
        spread = 9
        for i in range(count):
            angle = base + (i - (count - 1) / 2) * spread
            direction = Vec2(1, 0).rotate(angle)
            self.spawn_queue.append([i * 0.07, direction])
    ...
    def spawn_spiders(self):
        ...
        for i in range(6):
            ...
            start, end = wall_point(side, random.uniform(0.08, 0.92)), wall_point(
                other, random.uniform(0.08, 0.92)
            )
    ...
    def wall_bounce(self, fighter, target, away=False):
        ...
        if fighter.pos.x - fighter.radius < ARENA.left:
            fighter.pos.x, fighter.vel.x, normal, bounced = (
                ARENA.left + fighter.radius,
                abs(fighter.vel.x),
                Vec2(-1, 0),
                True,
            )
            ...
            fighter.pos.x, fighter.vel.x, normal, bounced = (
                ARENA.right - fighter.radius,
                -abs(fighter.vel.x),
                Vec2(1, 0),
                True,
            )
        if fighter.pos.y - fighter.radius < ARENA.top:
            fighter.pos.y, fighter.vel.y, normal, bounced = (
                ARENA.top + fighter.radius,
                abs(fighter.vel.y),
                Vec2(0, -1),
                True,
            )
        ...
        if bounced:
            fighter.squash = 0.4
            if (
                fighter is self.dark
                and fighter.mode == "normal"
                and fighter.portal_timer <= 0
    ...
    def update(self, dt):
        ...
        if d.mode == "portal":
            ...
            bot.pos.x = clamp(
                bot.pos.x, ARENA.left + bot.radius, ARENA.right - bot.radius
            )
            bot.pos.y = clamp(
                bot.pos.y, ARENA.top + bot.radius, ARENA.bottom - bot.radius
            )
            if d.mode_timer <= 0:
                d.mode = "normal"
            ...
            if d.pos.distance_to(bot.pos) <= d.radius + bot.radius + 34:
                ...
                d.mode, d.immobilized, d.vel = (
                    "normal",
                    2,
                    safe_normal(d.vel).reflect(
                        Vec2(1, 0)
                        if d.pos.x in (ARENA.left + d.radius, ARENA.right - d.radius)
                        else Vec2(0, 1)
                    )
                    * 350,
                )
        ...
        if d.mode == "normal" and not d.hidden and distance < d.radius + bot.radius:
            ...
            if bot.punch_timer <= 0 and bot.stunned <= 0:
                ...
                base_angle = math.degrees(
                    math.atan2(d.pos.y - bot.pos.y, d.pos.x - bot.pos.x)
                )
                bot.punch_dir = Vec2(1, 0).rotate(
        ...
        if (
            d.mode == "normal"
            and not d.hidden
            and distance <= d.radius + bot.radius + 125
            and d.attack_timer <= 0
            and not self.actions_locked
        ):
            self.slash()
        if (
        ...
        for pool in self.pools:
            if pool.hardened > 0:
                ...
                if pool.tick <= 0 and spike_hit:
                    pool.tick = 0.5
                    spike_damage = (
                        12 if getattr(self, "target_is_summon", False) else 37
                    )
                    self.damage_dummy(spike_damage, "VIRA SPIKES")
                    healed = min(spike_damage * 0.5, d.max_hp - d.hp)
                    d.hp += healed
                    if healed > 0:
                        self.text(f"+{healed:g} HP", d.pos + Vec2(0, -72), HOT_RED)
        expired = [
            p for p in self.pools if p.hardened <= 0 and p.hardened != 0 and p.melt <= 0
        ]
        for pool in expired:
            pool.hardened = 0
            pool.melt = 1.15
    ...
    def draw_electric_arc(self, layer, start, end, phase=0, alpha=220, width=2):
        ...
        for i in range(1, 7):
            t = i / 7
            jitter = math.sin(self.time * 42 + phase + i * 3.7) * (8 - abs(0.5 - t) * 8)
            points.append(start + direction * t + q * jitter)
        points.append(end)
        pygame.draw.lines(
            layer, (*DARK_RED, int(alpha * 0.55)), False, points, width + 4
        )
        pygame.draw.lines(layer, (*HOT_RED, alpha), False, points, width)
        pygame.draw.lines(layer, (255, 210, 165, int(alpha * 0.55)), False, points, 1)
    ...
    def draw_vira_blade(self, layer, origin, direction, scale=1, mirror=1, alpha=255):
        ...
        pygame.draw.lines(
            layer, (*DARK_RED, alpha), True, points, max(4, int(9 * scale))
        )
        pygame.draw.lines(
            layer, (*HOT_RED, int(alpha * 0.88)), True, points, max(2, int(3 * scale))
        )
        inner = [
        ...
        pygame.draw.polygon(layer, (32, 3, 12, int(alpha * 0.75)), inner)
        pygame.draw.lines(
            layer, (135, 13, 24, int(alpha * 0.92)), True, inner, max(2, int(3 * scale))
        )
        point = origin + direction * 166 * scale
        fork_a = origin + direction * 38 * scale + q * -48 * scale
        fork_b = origin + direction * 28 * scale + q * 42 * scale
        self.draw_electric_arc(
            layer, fork_a, point, 1.7, int(alpha * 0.86), max(1, int(2 * scale))
        )
        self.draw_electric_arc(
            layer, fork_b, point, 4.1, int(alpha * 0.8), max(1, int(2 * scale))
        )
        for i in range(5):
            spark = (
                point - direction * (8 + i * 7) + q * math.sin(self.time * 35 + i) * 7
            )
            pygame.draw.circle(
                layer,
                (*HOT_RED, int(alpha * 0.7)),
                spark,
                max(1, int((3 - i * 0.35) * scale)),
            )

    def draw_pool(self, dst, pool, offset):
        layer = pygame.Surface((W, H), pygame.SRCALPHA)
        melt_progress = clamp(pool.melt / 1.15, 0, 1)
        pulse = 1 + math.sin(self.time * 3 + pool.phase) * 0.07
        flatten = 1 - (1 - melt_progress) * 0.28 if pool.spent else 1
        points = [
            pool.pos
            + Vec2(p.x * pulse * (1.15 if pool.spent else 1), p.y * pulse * flatten)
            + offset
            for p in pool.points
        ]
        pygame.draw.polygon(layer, (3, 3, 10, 225), points)
        inner = [
            pool.pos + offset + (point - pool.pos - offset) * 0.68 for point in points
        ]
        ...
        if pool.hardened > 0 or pool.melt > 0:
            for i, point in enumerate(points):
                ...
                if pool.hardened > 0:
                    growth = clamp((7.25 - pool.hardened) / 0.25, 0, 1)
                    collapse = 1
                else:
                    growth = 1
                    collapse = melt_progress * melt_progress
                length = (
                    (34 + (i % 4) * 12 + math.sin(pool.phase + i) * 6)
                    * growth
                    * collapse
                )
                sag = Vec2(0, (1 - collapse) * (35 + i % 3 * 10))
                tip = point + direction * length + sag
                base = 11 * max(0.25, collapse)
                pygame.draw.polygon(
                    layer,
                    (5, 4, 11, int(255 * max(0.25, collapse))),
                    [point - side * base, point + side * base, tip],
                )
                pygame.draw.line(
                    layer,
                    (*DARK_RED, int(255 * collapse)),
                    point - side * base * 0.65,
                    tip,
                    5,
                )
                pygame.draw.line(
                    layer,
                    (*HOT_RED, int(245 * collapse)),
                    point - side * base * 0.3,
                    tip,
                    2,
                )
                if pool.melt > 0:
                    drip = tip + Vec2(
                        math.sin(i + self.time * 5) * 3, (1 - melt_progress) * 30
                    )
                    pygame.draw.line(
                        layer, (*DARK_RED, int(210 * melt_progress)), tip, drip, 3
                    )
                    pygame.draw.circle(
                        layer, (*HOT_RED, int(190 * melt_progress)), drip, 3
                    )
        dst.blit(layer, (0, 0))
    ...
    def draw_portal(self, dst, portal, offset):
        ...
        q = Vec2(-n.y, n.x)
        layer = pygame.Surface((W, H), pygame.SRCALPHA)
        open_amount = math.sin(math.pi * t) ** 0.55
        points = []
        for i in range(36):
            angle = i / 36 * math.tau
            wobble = 1 + math.sin(angle * 6 + self.time * 18) * 0.16
            points.append(
                p
                + q * math.cos(angle) * 88 * open_amount * wobble
                + n * math.sin(angle) * 22
            )
        ...
        for i in range(16):
            ...
            mid = (
                root
                + n * (23 + i % 4 * 9)
                + q * side * math.sin(self.time * 14 + i) * 11
            )
            tip = mid + n * (13 + i % 3 * 7) - q * side * 5
            pygame.draw.lines(
                layer, (*DARK_RED, int(200 * t)), False, [root, mid, tip], 7
            )
            pygame.draw.lines(
                layer, (*HOT_RED, int(185 * t)), False, [root, mid, tip], 2
            )
            pygame.draw.circle(layer, (*HOT_RED, int(190 * t)), tip, 3)
        for radius in (24, 42, 61):
            rect = pygame.Rect(
                p.x - radius, p.y - radius * 0.32, radius * 2, radius * 0.64
            )
            pygame.draw.arc(
                layer,
                (*HOT_RED, int((150 - radius) * t)),
                rect,
                self.time * 5,
                self.time * 5 + math.pi * 1.3,
                2,
            )
        for i in range(5):
    ...
    def draw_spider(self, dst, spider, offset):
        p = spider.pos + offset
        direction, q = safe_normal(spider.vel), Vec2(
            -safe_normal(spider.vel).y, safe_normal(spider.vel).x
        )
        ...
        for i in range(1, 5):
            ...
            pygame.draw.ellipse(
                layer, (*DARK_RED, 24), pygame.Rect(echo.x - 10, echo.y - 17, 20, 34)
            )
        for side in (-1, 1):
            for j in (-1, 1):
                ...
                knee = (
                    root
                    + q * side * (42 + j * 8)
                    + direction * math.sin(phase + side * 2 + j) * 18
                )
                ankle = knee + q * side * 29 - direction * (18 + j * 10)
                tip = ankle + direction * 13 + q * side * 8
                pygame.draw.lines(
                    layer, (20, 3, 10, 230), False, [root, knee, ankle, tip], 11
                )
                pygame.draw.lines(
                    layer, (*HOT_RED, 245), False, [root, knee, ankle, tip], 3
                )
                pygame.draw.circle(layer, (255, 150, 95, 220), knee, 3)
        pygame.draw.ellipse(
            layer, (22, 4, 12, 255), pygame.Rect(p.x - 15, p.y - 25, 30, 50)
        )
        pygame.draw.ellipse(layer, HOT_RED, pygame.Rect(p.x - 15, p.y - 25, 30, 50), 3)
        pygame.draw.ellipse(
            layer, (245, 45, 30, 150), pygame.Rect(p.x - 7, p.y - 15, 14, 23)
        )
        pygame.draw.line(
            layer, (255, 185, 120, 210), p - direction * 13, p + direction * 13, 2
        )
        for side in (-1, 1):
        ...
        for side in (-1, 1):
            ...
            pygame.draw.arc(
                layer,
                HOT_RED,
                pygame.Rect(claw.x - 10, claw.y - 10, 20, 25),
                -math.pi * 0.2 if side < 0 else math.pi * 0.7,
                math.pi * 0.8 if side < 0 else math.pi * 1.7,
                3,
            )
        if int(spider.age * 16) % 3 == 0:
            self.draw_electric_arc(
                layer,
                p - direction * 12,
                p - direction * 38 + q * math.sin(phase) * 8,
                spider.phase,
                130,
                1,
            )
    ...
    def draw_ball(self, dst, fighter, offset, darklord=False):
        def decorate(ball, center, radius, facing, roll):
            ...
            pygame.draw.arc(ball, (18, 5, 12), band_rect, roll + 0.35, roll + 1.42, 7)
            pygame.draw.arc(ball, HOT_RED, band_rect, roll + 0.42, roll + 1.34, 2)
            pygame.draw.arc(
                ball,
                (18, 5, 12),
                band_rect,
            ...
            eye_center = center + facing * radius * 0.39
            q = Vec2(-facing.y, facing.x)
            for side in (-1, 1):
                eye = eye_center + q * side * radius * 0.18
                pygame.draw.line(
                    ball, VIRA, eye - facing * 7 - q * side * 5, eye + facing * 7, 6
                )
                pygame.draw.line(
                    ball, (255, 225, 180), eye + facing * 1, eye + facing * 6, 2
                )
        ...
        shared.draw_ball(
            ...
            fighter.facing,
            fighter.roll if darklord else self.time * 2,
            fighter.squash,
            fighter.hit_flash,
    ...
    def draw_blades(self, dst, offset):
        ...
        if d.stab_anim > 0:
            for side in (-1, 1):
                ...
                self.draw_vira_blade(layer, root, direction, 0.92, side)
                for i in range(5):
                    a = (
                        root
                        - direction * (10 + i * 12)
                        + q * side * math.sin(self.time * 24 + i) * 6
                    )
            ...
            for i in range(11):
                ...
                start = dark_center + Vec2(1, 0).rotate_rad(angle) * (
                    d.radius + 4 + i % 3 * 5
                )
                end = dummy_center + Vec2(1, 0).rotate_rad(-angle * 1.3) * (
                    self.dummy.radius + 5 + i % 4 * 4
                )
                self.draw_electric_arc(
                    layer,
                    start,
            ...
            for center, radius, reverse in (
                (dark_center, d.radius + 13, 1),
                (dummy_center, self.dummy.radius + 14, -1),
            ):
                ring = []
                for i in range(18):
            ...
            for i in range(5, 0, -1):
                ...
                pygame.draw.line(
                    layer, (*DARK_RED, 15 + (6 - i) * 10), center, echo_tip, 8 - i
                )
            ...
            for i in range(5):
                spark = (
                    self.dummy.pos
                    + offset
                    + Vec2(1, 0).rotate(self.time * 280 + i * 72) * (35 + i * 4)
                )
    ...
    def draw_fire_cast(self, dst, offset):
        if self.fire_cast <= 0:
            return
        t = clamp(self.fire_cast / 0.42, 0, 1)
        center = self.dark.pos + offset - Vec2(0, 72)
        layer = pygame.Surface((W, H), pygame.SRCALPHA)
        radius = 28 + (1 - t) * 28
        for i in range(3):
            r = radius + i * 10
            rect = pygame.Rect(center.x - r, center.y - r * 0.38, r * 2, r * 0.76)
            pygame.draw.arc(
                layer,
                (*HOT_RED, int(220 * t)),
                rect,
                self.time * (5 + i),
                self.time * (5 + i) + math.pi * (1.15 + i * 0.15),
                3,
            )
        for i in range(8):
            angle = self.time * 5 + i / 8 * math.tau
            a = center + Vec2(radius, 0).rotate_rad(angle)
            b = center + Vec2(radius + 17, 0).rotate_rad(angle + 0.22)
            pygame.draw.line(layer, (*RED, int(170 * t)), a, b, 3)
        pygame.draw.circle(layer, (4, 3, 10, int(230 * t)), center, int(radius * 0.62))
        pygame.draw.circle(
            layer, (*HOT_RED, int(240 * t)), center, int(radius * 0.62), 3
        )
        dst.blit(layer, (0, 0))
    ...
    def draw_hud(self, dst, fonts):
        ...
        dst.blit(
            fonts["small"].render(f"{int(self.dark.hp)} / 5000", True, (255, 190, 175)),
            (80, 70),
        )
        ...
        for i in range(10):
            pygame.draw.circle(
                dst,
                RED if i < self.dark.stacks else (55, 22, 30),
                (W // 2 - 81 + i * 18, 55),
                6,
            )
        status = []
        ...
        skills = [
            ("VIRA-PORTAL", self.dark.portal_timer, 9),
            ("DARK ENERGY", self.dark.fire_timer, 8),
        ...
        for i, (name, cooldown, total) in enumerate(skills):
            ...
            pygame.draw.rect(
                dst,
                RED,
                (x, y, int(170 * (1 - clamp(cooldown / total, 0, 1))), 7),
                border_radius=4,
            )
        info = fonts["tiny"].render(
            "R  RESTART     M  MUTE     ESC  EXIT", True, (130, 95, 110)
        )
        ...
        for fire in self.fireballs:
            ...
            for i in range(11, 0, -1):
                trail = (
                    center
                    - tail * i * 12
                    + q * math.sin(fire.age * 31 + i * 1.7) * (4 + i * 1.2)
                )
                radius = max(2, int(fire.radius * (1 - i / 14)))
                pygame.draw.circle(
                    fire_layer, (*DARK_RED, 22 + (12 - i) * 11), trail, radius
                )
                pygame.draw.circle(
                    fire_layer,
                    (*HOT_RED, 32 + (12 - i) * 7),
            ...
            pygame.draw.circle(
                fire_layer, (3, 3, 9, 255), center, int(fire.radius * 0.88)
            )
            pygame.draw.circle(
                fire_layer, (86, 7, 22, 235), center, int(fire.radius * 0.67), 5
            )
            pygame.draw.circle(fire_layer, HOT_RED, center, int(fire.radius * 0.88), 3)
            pygame.draw.circle(
                fire_layer, (255, 165, 100, 220), center - tail * 6 - q * 6, 5
            )
            for i in range(3):
                orbit_angle = fire.age * (7 + i * 1.7) + i * math.tau / 3
                satellite = center + Vec2(1, 0).rotate_rad(orbit_angle) * (
                    fire.radius + 10 + i * 4
                )
                pygame.draw.circle(fire_layer, (*HOT_RED, 210), satellite, 3 + i)
            for i in range(5):
                a = center - tail * (fire.radius * 0.4 + i * 5) + q * (-18 + i * 9)
                b = (
                    center
                    - tail * (fire.radius + 35 + i * 8)
                    + q * math.sin(fire.age * 42 + i) * 21
                )
                self.draw_electric_arc(
                    fire_layer, a, b, i * 2.3, 195 - i * 18, 1 + (i % 2)
                )
        ...
        if self.dark.mode == "dash":
            ...
            for i in range(1, 12):
                p = (
                    ...
                    + offset
                    - direction * i * 17
                    + q * math.sin(self.time * 33 + i * 1.8) * (8 + i)
                )
                pygame.draw.circle(
                    dash_layer, (*DARK_RED, max(8, 78 - i * 6)), p, max(2, 16 - i)
                )
                if i % 2:
                    pygame.draw.circle(
                        dash_layer,
                        (*HOT_RED, max(10, 125 - i * 9)),
                        p + q * (8 + i),
                        max(2, 6 - i // 3),
                    )
            for i in range(4):
                ...
                b = (
                    a
                    - direction * (38 + i * 9)
                    + q * math.sin(self.time * 48 + i * 2) * 28
                )
    ...
    def draw(self, dst, fonts):
        ...
        if self.banner_time > 0:
            image = fonts["banner"].render(
                "DARKLORD // AUTONOMOUS COMBAT PROTOTYPE", True, (255, 190, 175)
            )
