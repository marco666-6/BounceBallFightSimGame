...
class Particle:
    ...
    def draw(self, dst, offset):
        ...
        p = self.pos + offset
        if self.kind == "ring":
            pygame.draw.circle(dst, (*self.color, int(190 * t)), p, int(self.size * (2 - t)), max(1, int(3 * t)))
        elif self.kind == "smoke":
            pygame.draw.circle(dst, (*self.color, int(72 * t)), p, int(self.size * (1.7 - t)))
            if self.size > 12:
                pygame.draw.circle(dst, (255, 255, 255, int(22 * t)), p - Vec2(self.size * .18, self.size * .2),
                                   max(2, int(self.size * .38)))
        elif self.kind == "flame":
            direction = safe_normal(self.vel)
            q = Vec2(-direction.y, direction.x)
            points = [p + direction * self.size * 2.3, p - direction * self.size * 1.2 + q * self.size,
                      p - direction * self.size * .45, p - direction * self.size * 1.2 - q * self.size]
            pygame.draw.polygon(dst, (*self.color, int(185 * t)), points)
            pygame.draw.circle(dst, (255, 238, 165, int(120 * t)), p - direction * self.size * .2,
                               max(1, int(self.size * .32)))
        elif self.kind == "shard":
            direction = safe_normal(self.vel)
            q = Vec2(-direction.y, direction.x)
            points = [p + direction * self.size * 2.4, p - direction * self.size * .9 + q * self.size * .65,
                      p - direction * self.size * .9 - q * self.size * .65]
            pygame.draw.polygon(dst, (*self.color, int(220 * t)), points)
        elif self.kind == "orb":
            glow_circle(dst, p, self.size * (.5 + t * .7), self.color, t)
        else:
            tail = p - safe_normal(self.vel) * self.size * 4
            pygame.draw.line(dst, (*self.color, int(230 * t)), tail, p, max(1, int(self.size * t)))
...
class Naruto:
    ...
    facing: Vec2 = field(default_factory=lambda: Vec2(1, 0))
    punch_timer: float = .08
    shuriken_timer: float = .22
    clone_cd: float = 1.2
    rasengan_cd: float = 3.4
    keikaku_cd: float = 6
    keikaku_active: float = 0
    keikaku_tick: float = .5
    ...
    def __init__(self, muted=False):
        ...
        try:
            ...
            for key, name in names.items():
                ...
                sound.set_volume(.34 if key.startswith("throw") else .42)
...
class Battle:
    ...
    def burst(self, pos, color=CHAKRA, amount=16, speed=260, kind="spark", size=4):
        for _ in range(amount):
            ...
            self.particles.append(Particle(Vec2(pos), direction * random.uniform(speed * .25, speed),
                                           random.uniform(.25, .85), .85, random.uniform(size * .55, size * 1.45),
                                           color, kind))

    def smoke(self, pos, amount=24, color=(232, 232, 224), speed=130, size=(10, 28)):
        for _ in range(amount):
            direction = Vec2(1, 0).rotate(random.random() * 360)
            self.particles.append(Particle(Vec2(pos) + direction * random.uniform(2, 38),
                                           direction * random.uniform(35, speed),
                                           random.uniform(.45, 1.15), 1.15,
                                           random.uniform(size[0], size[1]), color, "smoke"))

    def text(self, value, pos, color=GOLD, big=False):
        self.texts.append(FloatText(value, Vec2(pos), color, 1 if big else .78, big))
    ...
    def impact(self, pos, color=CHAKRA, power=1):
        ...
        self.particles.append(Particle(Vec2(pos), Vec2(), .3, .3, 18 + power * 6, color, "ring"))
    ...
    def rasengan_blast_fx(self, pos, direction):
        ...
        for _ in range(58):
            ...
            bias = direction * random.uniform(.25, 1.1) + q * random.uniform(-.75, .75)
            vel = safe_normal(out + bias) * random.uniform(280, 820)
            color = random.choice((CHAKRA, CHAKRA_WHITE, (255, 221, 106), (255, 133, 48)))
            kind = random.choice(("spark", "shard", "flame"))
            self.particles.append(Particle(pos + out * random.uniform(6, 38), vel,
                                           random.uniform(.28, .95), .95,
                                           random.uniform(3, 9), color, kind, .91))
        for i in range(4):
            self.particles.append(Particle(pos, Vec2(), .32 + i * .08, .32 + i * .08,
                                           32 + i * 18, CHAKRA_WHITE if i % 2 else CHAKRA, "ring"))
    ...
    def active_clones(self):
        return [clone for clone in self.clones if clone.hp > 0 and clone.death_anim <= 0]
    ...
    def kagebunshin(self):
        ...
        for i in range(create_count):
            angle = (i / max(1, create_count)) * math.tau + random.uniform(-.35, .35)
            offset = Vec2(math.cos(angle), math.sin(angle)) * random.uniform(74, 112)
            pos = Vec2(clamp(n.pos.x + offset.x, ARENA.left + n.radius, ARENA.right - n.radius),
                       clamp(n.pos.y + offset.y, ARENA.top + n.radius, ARENA.bottom - n.radius))
            direction = safe_normal(offset, Vec2(1, 0)).rotate(random.uniform(-45, 45))
            self.clones.append(Clone(pos, direction * NARUTO_SPEED))
            self.particles.append(Particle(Vec2(pos), Vec2(), .35, .35, 32, CHAKRA_WHITE, "ring"))
    ...
    def rasengan_hit(self):
        ...
        self.text("MASSIVE KNOCKBACK", self.dummy.pos + Vec2(0, -98), CHAKRA_WHITE, True)
        self.shake = max(self.shake, 18)
        self.hit_stop = max(self.hit_stop, .07)
    ...
    def keikaku_tick(self):
        ...
        self.text(f"+{int(gained)} CHAKRA", self.naruto.pos + Vec2(0, -116), CHAKRA_WHITE, True)
        self.burst(self.naruto.pos, CHAKRA_WHITE, 18, 245, "spark", 4)
        self.particles.append(Particle(Vec2(self.naruto.pos), Vec2(), .4, .4, 46, CHAKRA, "ring"))
        ...
        if bounced:
            ...
            if isinstance(fighter, Clone):
                ...
                if fighter.wall_bounces % 2 == 0:
                    fighter.vel = safe_normal(self.dummy.pos - fighter.pos) * NARUTO_SPEED
                    self.text("CLONE REDIRECT", fighter.pos + Vec2(0, -58), CLONE_ORANGE)
    ...
    def update_naruto_actions(self, dt):
        ...
        if n.keikaku_active > 0:
            ...
            for _ in range(2):
                ...
                self.particles.append(Particle(start, inward * random.uniform(90, 175),
                                               random.uniform(.35, .72), .72,
                                               random.uniform(2, 4.5), color,
                                               "orb", .96))
        ...
        if n.clone_cd <= 0 and n.chakra >= 300:
            self.kagebunshin()
            return
        if n.rasengan_cd <= 0 and n.chakra >= 500 and not n.rasengan_ready and n.rasengan_charge <= 0:
            self.start_rasengan()
            return
        if n.keikaku_cd <= 0:
    ...
    def update_clone_actions(self):
        for clone in self.active_clones():
            if clone.stunned > 0:
                continue
            distance = clone.pos.distance_to(self.dummy.pos) - clone.radius - self.dummy.radius
    ...
    def update_dummy_knockback(self, dt):
        ...
        if hit_wall and self.dummy_knockback_stun_armed:
            ...
            self.text("WALL STUN  2.0s", self.dummy.pos + Vec2(0, -96), CHAKRA_WHITE, True)
    ...
    def update(self, dt):
        ...
        for clone in self.clones:
            clone.timers(dt)
        for group in (self.particles, self.texts, self.fists, self.rasengan_impacts, self.wall_cracks, self.shield_pops):
            for item in group:
                item.life -= dt
        ...
        for clone in self.active_clones():
            clone.facing += (safe_normal(clone.vel, clone.facing) - clone.facing) * min(1, dt * 9)
        ...
        naruto_body_speed = NARUTO_SPEED if n.keikaku_active <= 0 and n.stunned <= 0 else 0
        ...
        for clone in self.active_clones():
            if self.resolve_collision(clone, d, NARUTO_SPEED, d.vel.length()):
                if d.punch_timer <= 0 and d.stunned <= 0:
                    ...
                    base = math.degrees(math.atan2(clone.pos.y - d.pos.y, clone.pos.x - d.pos.x))
                    d.punch_dir = Vec2(1, 0).rotate(base + random.choice((-125, -75, 75, 125)))
                    self.sound.play("dummy_punch")
        ...
        for clone in self.active_clones():
            if n.pos.distance_to(clone.pos) < n.radius + clone.radius:
                naruto_speed = NARUTO_SPEED if n.keikaku_active <= 0 and n.stunned <= 0 else 0
    ...
    def draw_naruto_decor(self, ball, center, radius, facing, roll):
        ...
        forehead = center + facing * radius * .12 - q * radius * .03
        band = pygame.Rect(center.x - radius * .58, center.y - radius * .61, radius * 1.16, radius * .42)
        pygame.draw.arc(ball, (36, 42, 58), band, roll + .2, roll + math.pi * 1.08, 7)
        pygame.draw.arc(ball, CHAKRA_WHITE, band.inflate(-4, -4), roll + .28, roll + math.pi * .96, 2)
        pygame.draw.circle(ball, (60, 68, 82), forehead, int(radius * .13))
        pygame.draw.circle(ball, (186, 198, 210), forehead, int(radius * .1), 2)
        for side in (-1, 1):
            cheek = center + facing * radius * .48 + q * side * radius * .31
            for k in (-1, 0, 1):
                start = cheek - facing * radius * .03 + Vec2(0, k * radius * .08)
                end = cheek + q * side * radius * .22 + Vec2(0, k * radius * .055)
                pygame.draw.line(ball, (72, 35, 20), start, end, max(1, int(radius * .035)))
    ...
    def draw_chakra_aura(self, dst, offset):
        ...
        if intensity < .15 and n.keikaku_active <= 0 and not n.rasengan_ready:
            return
        layer = pygame.Surface((W, H), pygame.SRCALPHA)
        center = n.pos + offset
        if n.keikaku_active > 0:
            pulse = (math.sin(self.time * 7) + 1) * .5
            aura_radius = n.radius + 18 + pulse * 5
            glow_circle(layer, center, 26 + pulse * 5, CHAKRA, .22)
            pygame.draw.circle(layer, (*CHAKRA, 88), center, int(aura_radius), 2)
            pygame.draw.circle(layer, (*CHAKRA_WHITE, 56), center, int(aura_radius * .72), 1)
            for i in range(8):
                angle = self.time * 1.6 + i * math.tau / 8
                p = center + Vec2(math.cos(angle), math.sin(angle)) * (n.radius + 16 + math.sin(self.time * 5 + i) * 4)
        ...
        for i in range(streaks):
            angle = self.time * (1.8 + i % 3 * .25) + i * math.tau / streaks
            ...
            end = center + direction * (radius + 22 + intensity * 24) + tangent * (14 + intensity * 18)
            pygame.draw.line(layer, (*CHAKRA, int(38 + intensity * 86)), start, end, 2 + int(intensity * 2))
            pygame.draw.circle(layer, (*CHAKRA_WHITE, int(70 * intensity)), end, 2 + i % 2)
        dst.blit(layer, (0, 0), special_flags=pygame.BLEND_ADD)
    ...
    def draw_rasengan_charge(self, dst, offset):
        ...
        if n.rasengan_charge <= 0 and not n.rasengan_ready:
            return
        progress = 1 if n.rasengan_ready else 1 - clamp(n.rasengan_charge / .6, 0, 1)
        center = n.pos + offset + safe_normal(n.facing) * (n.radius + 20)
        radius = 14 + progress * 16 + math.sin(self.time * 24) * 2
        layer = pygame.Surface((W, H), pygame.SRCALPHA)
        for i in range(4):
            rect = pygame.Rect(center.x - radius - i * 7, center.y - radius - i * 7,
                               (radius + i * 7) * 2, (radius + i * 7) * 2)
            pygame.draw.arc(layer, (*CHAKRA, 185 - i * 30), rect,
                            self.time * (8 + i) + i, self.time * (8 + i) + i + math.pi * 1.3, 3)
        pygame.draw.circle(layer, (232, 252, 255, 210), center, int(radius * .58))
        pygame.draw.circle(layer, (*CHAKRA, 235), center, int(radius), 4)
        for i in range(14):
            angle = self.time * (9 + i % 3) + i / 14 * math.tau
            a = center + Vec2(math.cos(angle), math.sin(angle)) * radius * .38
            b = center + Vec2(math.cos(angle + .35), math.sin(angle + .35)) * (radius + 18 + i % 4 * 4)
    ...
    def draw_fists(self, dst, offset):
        for fist in self.fists:
            ...
            fade = clamp(min(progress / .1, (1 - progress) / .22), 0, 1)
            direction = safe_normal(fist.direction)
            q = Vec2(-direction.y, direction.x)
            target = fist.target + offset
            palm = target + direction * lerp(-128, 118, progress * progress * (3 - 2 * progress))
            ...
            for i in range(5, 0, -1):
                ...
                pygame.draw.ellipse(layer, (*fist.color, int(alpha * .045 * (6 - i))),
                                    pygame.Rect(echo.x - 28, echo.y - 22, 56, 44))
            forearm = [base - q * 15, wrist - q * 20, palm - direction * 19 - q * 24,
                       palm - direction * 19 + q * 24, wrist + q * 20, base + q * 15]
            pygame.draw.polygon(layer, (35, 45, 64, int(alpha * .75)), forearm)
            pygame.draw.line(layer, (*fist.color, int(alpha * .72)), base - q * 7, palm - direction * 20 - q * 14, 4)
            glove = [palm - direction * 25 - q * 25, palm + direction * 12 - q * 31,
                     palm + direction * 31 - q * 18, palm + direction * 31 + q * 19,
                     palm + direction * 10 + q * 31, palm - direction * 27 + q * 23]
            pygame.draw.polygon(layer, (*ORANGE, alpha), glove)
            pygame.draw.lines(layer, (255, 234, 184, int(alpha * .85)), True, glove, 3)
            for side in (-21, -7, 7, 21):
                knuckle = palm + direction * 30 + q * side
                pygame.draw.circle(layer, (255, 235, 170, alpha), knuckle, 10)
                pygame.draw.circle(layer, (*DEEP_ORANGE, int(alpha * .9)), knuckle + direction * 2, 6)
            for i in range(7):
                spark = wrist - direction * (i * 13) + q * math.sin(self.time * 12 + i) * 14
                pygame.draw.circle(layer, (*CHAKRA, int(alpha * .32)), spark, 2 + i % 2)
            dst.blit(layer, (0, 0))
    ...
    def draw_shuriken(self, dst, offset):
        ...
        for s in self.shuriken:
            ...
            for i in range(1, 5):
                ...
                pygame.draw.circle(layer, (185, 205, 225, max(10, 58 - i * 10)), trail, max(2, 7 - i))
            for blade in range(4):
                angle = s.angle + blade * math.pi / 2
                direction = Vec2(math.cos(angle), math.sin(angle))
                q = Vec2(-direction.y, direction.x)
                points = [center + direction * 18, center - direction * 4 + q * 7, center - direction * 4 - q * 7]
                pygame.draw.polygon(layer, (33, 39, 50, 240), points)
                pygame.draw.line(layer, (235, 242, 248, 190), center, center + direction * 15, 2)
    ...
    def draw_rasengan_impacts(self, dst, offset):
        for impact in self.rasengan_impacts:
            ...
            for i in range(7):
                ...
                pygame.draw.circle(layer, (*CHAKRA, int((120 - i * 12) * fade)), center, int(radius), 4)
                start = self.time * (8 + i) + i
                pygame.draw.arc(layer, (*CHAKRA_WHITE, int((210 - i * 22) * fade)),
                                pygame.Rect(center.x - radius, center.y - radius, radius * 2, radius * 2),
                                start, start + math.pi * (1.0 + i * .2), 3)
            for i in range(18):
    ...
    def draw_wall_cracks(self, dst, offset):
        ...
        for crack in self.wall_cracks:
            ...
            for i in range(9):
                direction = safe_normal(normal * random.Random(i).uniform(.2, 1.0) +
                                        q * random.Random(i + 99).uniform(-1, 1))
                length = (28 + i * 9) * t
                pygame.draw.line(layer, (235, 245, 255, int(210 * t)), p, p + direction * length, 3)
                pygame.draw.line(layer, (*CHAKRA, int(135 * t)), p + direction * 8, p + direction * length, 1)
            pygame.draw.circle(layer, (*CHAKRA_WHITE, int(85 * t)), p, int(50 * (1.2 - t)), 3)
        dst.blit(layer, (0, 0), special_flags=pygame.BLEND_ADD)
    ...
    def draw_shields(self, dst, offset):
        ...
        for shield in self.shield_pops:
            ...
            pygame.draw.circle(layer, (*CHAKRA_WHITE, int(125 * t)), p, int(radius * .68), 2)
        dst.blit(layer, (0, 0), special_flags=pygame.BLEND_ADD)

    def draw_status_icons(self, dst, offset):
        entries = []
        if self.naruto.stunned > 0:
            entries.append((self.naruto.pos, self.naruto.radius, self.naruto.stunned, CHAKRA_WHITE))
        if self.dummy.stunned > 0:
            entries.append((self.dummy.pos, self.dummy.radius, self.dummy.stunned, GOLD))
        for clone in self.active_clones():
            if clone.stunned > 0:
        ...
        for pos, radius, duration, color in entries:
            ...
            for i in range(5):
                ...
                pygame.draw.line(layer, (255, 255, 235, 210), p - Vec2(6, 0), p + Vec2(6, 0), 2)
        dst.blit(layer, (0, 0))
    ...
    def draw_hud(self, dst, fonts):
        ...
        dst.blit(fonts["small"].render(f"{int(n.hp)} / 5000", True, (255, 232, 190)), (80, 70))
        ...
        pygame.draw.rect(dst, CHAKRA, (chakra.x, chakra.y, int(chakra.width * n.chakra / n.max_chakra), chakra.height),
                         border_radius=6)
        pygame.draw.rect(dst, CHAKRA_WHITE, chakra, 1, border_radius=6)
        dst.blit(fonts["tiny"].render(f"CHAKRA {int(n.chakra)} / 1000    CLONES {len(self.active_clones())}/6",
                                      True, CHAKRA_WHITE), (80, 102))
        title = fonts["tiny"].render("NINJA PUNCH  //  SHURIKEN  //  KAGEBUNSHIN  //  RASENGAN  //  KEIKAKU",
                                     True, (198, 221, 235))
    ...
    def draw(self, dst, fonts):
        ...
        for clone in self.clones:
            ...
            shared.draw_ball(dst, clone.pos + offset, radius, (194, 108, 63), CLONE_ORANGE,
                             clone.facing, clone.roll, clone.squash, clone.hit_flash, 0, 0,
                             (255, 230, 175), self.draw_naruto_decor)
            if clone.hp > 0:
                self.draw_clone_bar(dst, clone, offset)
        shared.draw_ball(dst, self.naruto.pos + offset, self.naruto.radius, DEEP_ORANGE, GOLD,
                         self.naruto.facing, self.naruto.roll, self.naruto.squash, self.naruto.hit_flash,
                         0, 0, CHAKRA_WHITE, self.draw_naruto_decor)
        ...
        if self.banner_time > 0:
            image = fonts["banner"].render("NARUTO // AUTONOMOUS COMBAT PROTOTYPE", True, CHAKRA_WHITE)
