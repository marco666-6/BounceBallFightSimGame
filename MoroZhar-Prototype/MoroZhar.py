...
class Particle:
    ...
    def draw(self, dst, offset):
        ...
        if self.kind == "spark":
            ...
            pygame.draw.line(dst, mix(self.color, (255, 255, 255), 0.35), tail, p, max(1, int(self.size * t)))
        elif self.kind == "wind":
            direction = safe_normal(self.vel)
            tail = p - direction * self.size * (5 + t * 6)
            q = Vec2(-direction.y, direction.x)
            pygame.draw.line(dst, (*self.color, int(150 * t)), tail, p, max(1, int(self.size * .35)))
            pygame.draw.line(dst, (*ICE_WHITE, int(70 * t)), tail + q * 2, p + q * 2, 1)
        elif self.kind == "ring":
            pygame.draw.circle(dst, (*self.color, int(180 * t)), p, int(self.size * (2 - t)), max(1, int(3 * t)))
        elif self.kind == "shard":
            d = safe_normal(self.vel)
            q = Vec2(-d.y, d.x)
            pts = [p + d * self.size * 2, p - d * self.size, p + q * self.size * .6, p - q * self.size * .6]
            pygame.draw.polygon(dst, (*self.color, int(220 * t)), pts)
        else:
            glow_circle(dst, p, self.size * (0.4 + t * .7), self.color, t)
    ...
    def __init__(self, muted=False):
        ...
        try:
            ...
            paths = {
                ...
                "breath": ROOT / "MoroZhar-SoundEffects" / "MoroZhar-FreezingBreath.mp3",
                "ice_break": ROOT / "MoroZhar-SoundEffects" / "MoroZhar-IceBreakForEnemy.mp3",
                "vision1": ROOT / "MoroZhar-SoundEffects" / "MoroZhar-HeatVisionOneStack.mp3",
                "vision2": ROOT / "MoroZhar-SoundEffects" / "MoroZhar-HeatVisionTwoStack.mp3",
                "vision3": ROOT / "MoroZhar-SoundEffects" / "MoroZhar-HeatVisionThreeStack.mp3",
                "dummy_punch": ROOT / "DummyBot-SoundEffects" / "MoroZhar-Punch.mp3",
            }
            for key, path in paths.items():
                sound = pygame.mixer.Sound(path)
                sound.set_volume(.42 if "vision" not in key else .32)
...
class Battle:
    ...
    def burst(self, pos, color, amount=18, speed=280, kind="spark", size=4):
        for _ in range(amount):
            ...
            s = speed * random.uniform(.25, 1)
            self.particles.append(Particle(Vec2(pos), Vec2(math.cos(a), math.sin(a)) * s,
                                           random.uniform(.25, .7), .7, random.uniform(size * .5, size * 1.5), color, kind))

    def text(self, text, pos, color, big=False):
        self.texts.append(FloatText(text, Vec2(pos), color, 1.0 if big else .75, big))

    def impact(self, pos, color, power=1):
        self.shake = max(self.shake, 5 + power * 4)
        self.hit_stop = max(self.hit_stop, .018 + power * .018)
        self.burst(pos, color, 12 + power * 7, 220 + power * 100, "spark", 3 + power)
        self.particles.append(Particle(Vec2(pos), Vec2(), .3, .3, 18 + power * 5, color, "ring"))

    def damage_dummy(self, amount, label="", color=GOLD, power=1):
        dealt = self.dummy.take_damage(amount)
        self.text(f"-{int(dealt)}" + (f"  {label}" if label else ""), self.dummy.pos + Vec2(0, -62), color, power > 1)
        self.impact(self.dummy.pos, color, power)

    def damage_moro(self, amount):
        self.moro.hp = max(0, self.moro.hp - amount)
        self.moro.hit_flash = .12
    ...
    def freeze(self):
        ...
        for _ in range(28):
            ...
            self.particles.append(Particle(p, safe_normal(self.dummy.pos - p) * random.uniform(55, 150),
                                           .55, .55, random.uniform(3, 7), ICE_WHITE, "frost"))
        self.shake = 12
    ...
    def shatter_ice(self):
        ...
        self.hit_stop = max(self.hit_stop, .045)
    ...
    def punch(self):
        ...
        self.moro.attack_timer = .5
        self.moro.punch_anim = .52
        self.moro.punch_kind = kind
        self.moro.punch_target = Vec2(d.pos)
        base_angle = math.degrees(math.atan2(d.pos.y - self.moro.pos.y, d.pos.x - self.moro.pos.x))
        self.moro.punch_dir = Vec2(1, 0).rotate(base_angle + random.choice((-145, -105, -65, 65, 105, 145)))
    ...
    def cast_vision(self):
        ...
        self.text(f"HEAT VISION  //  LEVEL {stacks}", self.moro.pos + Vec2(0, -82), HEAT, True)

    def wall_bounce(self, fighter, seek_target, away=False):
        bounced = False
        if fighter.pos.x - fighter.radius < ARENA.left or fighter.pos.x + fighter.radius > ARENA.right:
            fighter.pos.x = clamp(fighter.pos.x, ARENA.left + fighter.radius, ARENA.right - fighter.radius)
            fighter.vel.x *= -1
            bounced = True
        if fighter.pos.y - fighter.radius < ARENA.top or fighter.pos.y + fighter.radius > ARENA.bottom:
            fighter.pos.y = clamp(fighter.pos.y, ARENA.top + fighter.radius, ARENA.bottom - fighter.radius)
            fighter.vel.y *= -1
            bounced = True
        if bounced:
            fighter.squash = .45
            self.burst(fighter.pos, (130, 170, 230), 7, 150, "spark", 2)
            if fighter.redirects > 0 and not (fighter is self.moro and self.actions_locked):
    ...
    def update(self, dt):
        ...
        if m.breath_timer <= 0 and self.breath <= 0 and self.vision <= 0 and in_breath_cone and not self.actions_locked:
            self.cast_breath()
        vision_should_cast = d.heat_stacks == 3 or m.vision_timer <= -2.5
        if (m.vision_timer <= 0 and d.heat_stacks > 0 and vision_should_cast and
                self.vision <= 0 and self.breath <= 0 and not self.actions_locked):
            self.cast_vision()
        ...
        if self.breath > 0:
            ...
            for _ in range(22):
                ...
                flow = self.breath_dir.rotate(random.uniform(-8, 8)) * random.uniform(220, 480)
                flow += q * random.uniform(-55, 55)
                color = ICE_WHITE if random.random() < .42 else ICE
                roll = random.random()
                kind = "wind" if roll < .72 else ("shard" if roll < .86 else "frost")
                self.particles.append(Particle(pos, flow, random.uniform(.3, .75), .75,
                                               random.uniform(2, 6), color, kind))
        ...
        if self.vision > 0:
            ...
            target_distance = ray_circle_hit_distance(origin, beam_dir, d.pos, d.radius + 12)
            self.vision_hit_target = target_distance is not None and target_distance <= wall_distance
            ...
            if self.vision_hit_target:
                ...
                if self.vision_tick <= 0:
                    ...
                    if d.should_thermal_shock():
                        self.thermal_shock()
                for _ in range(3):
                    self.particles.append(Particle(Vec2(self.vision_end), Vec2(random.uniform(-150, 150), random.uniform(-150, 150)),
            ...
            for _ in range(4):
                ...
                self.particles.append(Particle(p + side, -beam_dir * random.uniform(30, 90),
                                               .22, .22, random.uniform(2, 5), mix(HEAT, GOLD, random.random()), "orb"))
        ...
        if d.burned > 0:
            ...
            if random.random() < dt * 20:
                self.particles.append(Particle(d.pos + Vec2(random.uniform(-30, 30), random.uniform(-20, 30)),
                                               ...
                                               .5, .5, random.uniform(4, 8), HEAT, "orb"))
        if d.frozen > 0 and random.random() < dt * 12:
            self.particles.append(Particle(d.pos + Vec2(random.uniform(-42, 42), random.uniform(-42, 42)),
                                           Vec2(random.uniform(-15, 15), -30), .6, .6, random.uniform(3, 6), ICE, "frost"))
    ...
    def draw_world_effects(self, dst, offset):
        if self.breath > 0:
            ...
            for i in range(54):
                along = ((i * .081 + self.time * (.28 + i % 4 * .025)) % 1)
                center = origin + self.breath_dir * along * 430
                ...
                pygame.draw.line(wind, (*color, 42 + i % 4 * 12), tail, p, 1 + (i % 7 == 0))
                if i % 4 == 0:
        ...
        if self.vision > 0:
            ...
            glow_circle(dst, b, 18 if self.vision_hit_target else 11, HEAT, .8 if self.vision_hit_target else .38)
    ...
    def draw_character_aura(self, dst, offset):
        ...
        glow_circle(dst, tell_pos, 24, tell_color, .12)
        start = self.time * 2.8
        layer = pygame.Surface((W, H), pygame.SRCALPHA)
        pygame.draw.arc(layer, (*tell_color, 210),
                        pygame.Rect(tell_pos.x - tell_radius, tell_pos.y - tell_radius,
                                    tell_radius * 2, tell_radius * 2),
                        start, start + math.pi * .85, 3)
        pygame.draw.arc(layer, (*tell_color, 120),
                        pygame.Rect(tell_pos.x - tell_radius - 5, tell_pos.y - tell_radius - 5,
                                    (tell_radius + 5) * 2, (tell_radius + 5) * 2),
                        start + math.pi, start + math.pi * 1.72, 2)
        dst.blit(layer, (0, 0))

    def draw_ball(self, dst, pos, radius, base, accent, facing, roll, squash, flash, frozen=0, burned=0, moro=False):
        pos = Vec2(pos)
        ...
        for rr in range(int(radius), 0, -2):
            t = rr / radius
            light = clamp(1 - t, 0, 1)
            col = mix(base, (255, 255, 255), light * .45)
        ...
        for side in (-1, 1):
            ...
            pygame.draw.circle(ball, eye_col, eye, int(radius * .075))
        if moro:
            pygame.draw.arc(ball, ICE, pygame.Rect(r - radius * .67, r - radius * .66, radius * 1.34, radius * 1.34),
                            math.pi * .5, math.pi * 1.5, 4)
            pygame.draw.arc(ball, HEAT, pygame.Rect(r - radius * .67, r - radius * .66, radius * 1.34, radius * 1.34),
        ...
        if frozen > 0:
            ...
            for a in range(0, 360, 45):
                ...
                pygame.draw.line(ball, ICE_WHITE, c + Vec2(radius - 3, 0).rotate(a), tip, 3)
        if burned > 0:
    ...
    def draw_punch_hand(self, dst, offset):
        ...
        if remaining <= 0:
            return
        progress = clamp(1 - remaining / .52, 0, 1)
        if progress < .12:
            fade = progress / .12
        elif progress < .68:
            fade = 1.0
            ...
            fade = (1 - progress) / .32
        fade = clamp(fade, 0, 1) ** .7
        travel_t = progress * progress * (3 - 2 * progress)

        direction = safe_normal(self.moro.punch_dir)  # Incoming direction toward the target.
        q = Vec2(-direction.y, direction.x)
        ...
        for i in range(5, 0, -1):
            ...
            echo_alpha = int(alpha * .045 * (6 - i))
            pygame.draw.ellipse(layer, (*mid, echo_alpha),
                                pygame.Rect(echo.x - 25, echo.y - 20, 50, 40))
            pygame.draw.line(layer, (*light, int(echo_alpha * .8)),
                             echo - direction * 31 - q * 12, echo - q * 12, 2)
        ...
        pygame.draw.polygon(layer, (*dark, int(alpha * .78)), forearm)
        pygame.draw.polygon(layer, (*mid, int(alpha * .32)),
                            [base - q * 8, wrist - q * 13, wrist + q * 2, base + q * 5])
        pygame.draw.line(layer, (*light, int(alpha * .52)), base - q * 9, palm - direction * 19 - q * 15, 3)
        ...
        pygame.draw.polygon(layer, (*light, int(alpha * .18)),
                            [palm - direction * 18 - q * 18, palm + direction * 5 - q * 23,
                             palm + direction * 19 - q * 13, palm - direction * 2 - q * 5])
        pygame.draw.lines(layer, (*light, int(alpha * .8)), True, palm_poly, 3)
        ...
        for i, side in enumerate(knuckle_offsets):
            ...
            pygame.draw.circle(layer, (*light, int(alpha * .86)), center - direction * 5 - q * 3, 4)
        ...
        if kind == "ICE PUNCH":
            for side, length in ((-29, 28), (-13, 23), (7, 27), (27, 32)):
                ...
                tip = root + direction * length + q * side * .16
                pygame.draw.polygon(layer, (*ICE_WHITE, int(alpha * .9)),
                                    [root - q * 5, root + q * 5, tip])
            for i in range(9):
                trail = wrist - direction * (i * 14) + q * math.sin(self.time * 9 + i) * 13
                pygame.draw.polygon(layer, (*ICE_WHITE, int(alpha * .38)),
                                    [trail + direction * 5, trail - direction * 5 + q * 3,
                                     trail - direction * 5 - q * 3])
        elif kind == "HEAT PUNCH":
            for side, length in ((-27, 38), (-12, 29), (5, 43), (23, 34)):
                root = wrist - direction * 4 + q * side
                tip = root - direction * length + q * math.sin(self.time * 18 + side) * 6
                pygame.draw.polygon(layer, (*HEAT, int(alpha * .7)),
                                    [root - q * 8, root + q * 8, tip])
                inner = vlerp(root, tip, .55)
                pygame.draw.circle(layer, (*light, int(alpha * .62)), inner, 5)
            for i in range(7):
                ember = wrist - direction * (i * 16) + q * math.sin(i * 2.7 + self.time * 15) * 16
                pygame.draw.circle(layer, (*light, int(alpha * .5)), ember, 2 + i % 3)
        else:
            pygame.draw.line(layer, (*light, int(alpha * .8)), wrist - q * 15, wrist + q * 15, 5)
            for side in (-13, 0, 13):
                pygame.draw.line(layer, (*dark, int(alpha * .75)),
                                 palm - direction * 17 + q * side, palm + direction * 11 + q * side, 2)
    ...
    def draw_ice_cube(self, dst, pos, frozen):
        ...
        seed_points = [
            (-61, -32), (-49, -55), (-24, -67), (3, -61), (30, -68), (53, -49),
            (65, -20), (61, 8), (69, 31), (48, 57), (20, 64), (-9, 69),
            (-35, 61), (-58, 43), (-66, 13), (-64, -13),
        ]
        shell = [pos + Vec2(x, y).normalize() * (Vec2(x, y).length() + pulse) for x, y in seed_points]
        ice = pygame.Surface((W, H), pygame.SRCALPHA)
        pygame.draw.polygon(ice, (38, 137, 190, 188), shell)
        pygame.draw.polygon(ice, (104, 205, 235, 135),
                            [shell[0], shell[1], shell[2], shell[3], pos + Vec2(11, -14), pos + Vec2(-31, 7)])
        pygame.draw.polygon(ice, (18, 91, 150, 120),
                            [shell[5], shell[6], shell[7], shell[8], pos + Vec2(21, 18), pos + Vec2(14, -20)])
        pygame.draw.polygon(ice, (175, 236, 246, 118),
                            [shell[10], shell[11], shell[12], shell[13], pos + Vec2(-27, 12), pos + Vec2(4, 25)])
        pygame.draw.lines(ice, (205, 248, 255, 235), True, shell, 4)
        pygame.draw.lines(ice, (82, 188, 225, 220), True, [p + Vec2(0, 4) for p in shell], 2)

        # Chunky frost patches make the prison read as ice instead of clean glass.
        frost_chunks = [
            (-48, -38, 20), (-24, -55, 17), (9, -52, 22), (39, -39, 19),
            (51, -6, 18), (45, 27, 21), (21, 48, 19), (-13, 53, 23),
            (-43, 37, 20), (-52, 5, 18), (-20, -12, 15), (22, 12, 17),
        ]
        for x, y, radius in frost_chunks:
            p = pos + Vec2(x, y)
            pygame.draw.circle(ice, (190, 239, 247, 115), p, radius)
            pygame.draw.circle(ice, (238, 253, 255, 115), p + Vec2(-radius * .25, -radius * .3), int(radius * .45))
        for x, length, base_y in ((-48, 18, 55), (-24, 29, 58), (7, 19, 57), (31, 25, 56), (49, 15, 54)):
            base = pos + Vec2(x, base_y)
            pygame.draw.polygon(ice, (137, 222, 240, 220),
                                [base + Vec2(-6, 0), base + Vec2(6, 0), base + Vec2(1, length)])
        for angle, length in ((-145, 79), (-115, 88), (-72, 82), (-38, 91), (12, 77), (37, 85)):
            direction = Vec2(1, 0).rotate(angle)
            side = Vec2(-direction.y, direction.x)
            base = pos + direction * 58
            tip = pos + direction * length
            pygame.draw.polygon(ice, (119, 213, 237, 220), [base - side * 7, base + side * 7, tip])
        if frozen <= 2:
            crack_count = 3 + int((2 - frozen) * 3)
            for i in range(crack_count):
                angle = i * 2.19 + .4
                start = pos + Vec2(math.cos(angle), math.sin(angle)) * (10 + i % 3 * 5)
                mid = start + Vec2(math.cos(angle + .45), math.sin(angle + .45)) * 22
                end = mid + Vec2(math.cos(angle - .28), math.sin(angle - .28)) * 27
                pygame.draw.lines(ice, (245, 255, 255, 225), False, [start, mid, end], 2)
                pygame.draw.line(ice, (95, 180, 230, 150), mid, mid + Vec2(12, -9).rotate(i * 31), 1)
        dst.blit(ice, (0, 0))
        glow_circle(dst, pos, 18, ICE, .05)
    ...
    def draw_hud(self, dst, fonts):
        ...
        bar(pygame.Rect(75, 42, 430, 23), self.moro.hp, self.moro.max_hp, (75, 190, 235))
        bar(pygame.Rect(W - 505, 42, 430, 23), self.dummy.hp, self.dummy.max_hp, (185, 190, 205), True)
        ...
        for i in range(3):
            pygame.draw.circle(dst, ICE if i < self.dummy.ice_stacks else (35, 50, 70), (cx - 54 + i * 18, 54), 6)
            pygame.draw.circle(dst, HEAT if i < self.dummy.heat_stacks else (55, 42, 45), (cx + 20 + i * 18, 54), 6)
        status = []
        ...
        tell = fonts["small"].render(f"NEXT: {next_punch}", True, punch_colors[next_punch])
        ...
        info = fonts["tiny"].render("R  RESTART     M  MUTE     ESC  EXIT", True, (105, 125, 160))
    ...
    def draw(self, dst, fonts):
        ...
        pygame.draw.arc(tell_layer, (*tell_color, 210),
                        pygame.Rect(tell_pos.x - tell_radius, tell_pos.y - tell_radius,
                                    tell_radius * 2, tell_radius * 2),
                        start, start + math.pi * .85, 3)
        pygame.draw.arc(tell_layer, (*tell_color, 120),
                        pygame.Rect(tell_pos.x - tell_radius - 5, tell_pos.y - tell_radius - 5,
                                    (tell_radius + 5) * 2, (tell_radius + 5) * 2),
                        start + math.pi, start + math.pi * 1.72, 2)
        dst.blit(tell_layer, (0, 0))
        self.draw_ball(dst, self.moro.pos + offset, self.moro.radius, (28, 85, 130), (185, 235, 255),
                       self.moro.facing, self.moro.roll, self.moro.squash, self.moro.hit_flash, moro=True)
        self.draw_ball(dst, self.dummy.pos + offset, self.dummy.radius, (84, 91, 108), (180, 190, 205),
        ...
        tell_bg = pygame.Surface((tell.get_width() + 14, tell.get_height() + 6), pygame.SRCALPHA)
        pygame.draw.rect(tell_bg, (5, 8, 18, 190), tell_bg.get_rect(), border_radius=7)
        tell_bg.blit(tell, (7, 3))
        dst.blit(tell_bg, (tell_pos.x - tell_bg.get_width() / 2, tell_pos.y - self.moro.radius - 34))
        self.draw_hud(dst, fonts)

        if self.banner_time > 0:
            alpha = int(255 * clamp(self.banner_time / .5, 0, 1))
        ...
        if self.round_over:
            ...
            sub = fonts["small"].render("NEXT EXHIBITION STARTING...", True, (220, 230, 245))
